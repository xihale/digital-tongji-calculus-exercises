...
#let problems = (
  ...
  (
    ...
    answer: [$1$],
    solution: [
      取对数：$(ln x) ln(1+sin x) ~ (ln x) · sin x ~ x ln x -> 0$，故原极限 $e^0 = 1$。
    ],
  ),
  (
    kind: "blank",
  ...
  (
    ...
    solution: [
      设 $L$ 为所求极限，则 $ln L = lim_(x -> 0^+) (ln tan 3x)/(2 ln x)$。
      因 $tan 3x ~ 3x$，$ln tan 3x ~ ln(3x) = ln 3 + ln x$，故 $ln L = 1/2$，$L = sqrt(e)$。
    ],
  ),
  (
    kind: "compute",
    ...
    solution: [
      令 $u=sqrt(3x+2)$，则 $x=(u^2-2)/3$，$dif x = (2u\/3) dif u$，
      积分得 $-x + (4/3)sqrt(3x+2) - (4/3) ln(1+sqrt(3x+2)) + C$。
    ],
  ),
  (
    kind: "compute",
  ...
  (
    ...
    solution: [
      ...
      竖直渐近线 $x=0$；当 $x->plus.minus oo$ 有斜渐近线 $y=2x+7$
      （$e^(2/x)=1+2/x+2/x^2+o(1/x^2)$，故 $f(x)=(2x+3)(1+2/x+2/x^2+o(1/x^2))=2x+7+o(1)$）。
    ],
  ),
  ...
  (
    ...
    solution-parts: (
      ...
      [
        由 (1) 的 $xi$，对 $[0,xi]$ 与 $[xi,1]$ 分别用拉格朗日中值定理：
        $f(xi)-f(0)=f'(eta_1) xi$，$f(1)-f(xi)=f'(eta_2)(1-xi)$，
        即 $1-xi = f'(eta_1) xi$，$xi = f'(eta_2)(1-xi)$，
        两式相乘得 $f'(eta_1)f'(eta_2)=1$（且 $eta_1 != eta_2$）。
      ],
    ),
  ),
  (
