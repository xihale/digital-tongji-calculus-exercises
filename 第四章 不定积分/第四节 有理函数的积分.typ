...
1.有理函数也称为有理分式，整式也是有理分式的一种 (#answer-choice[])
   (#answer-choice[(正确)])

2.有理分式  $(x^3 + x^2 - x - 1)/(2x^3 + 3x^2 + 6x)$  是真分式  (#answer-choice[])
   (#answer-choice[(错误)])
   #answer-process[
...
3. 令  $t = tan(x/2)$ , 则  $integral (tan x)/(sin x + cos x - 1) dif x = integral (A)/((1 - t)(1 - t^2)) dif t$  中  $A = -2$ (#answer-choice[])
   (#answer-choice[(错误)])
   #answer-process[
     当 $t = tan(x/2)$ 时：
...
4. 在计算三角函数有理式的不定积分  $integral R(sin x, cos x) dif x$  时，一般使用变换  $t = tan(x/2)$ (#answer-choice[])
   (#answer-choice[(正确)])
   #answer-process[
     这是三角函数有理式的标准处理方法。
   ]

5.所有连续函数均存在初等函数的原函数 (#answer-choice[])
   (#answer-choice[(错误)])
   #answer-process[
