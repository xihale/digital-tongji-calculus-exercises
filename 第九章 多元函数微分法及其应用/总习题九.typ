...
#let problems = (
  ...
  (
    ...
    solution: [
      先有 $f(0,0)=0$。 \
      对 $(x,y)!=(0,0)$， \
      $|f(x,y)| = (x^2 y^2)/((x^2+y^2)^((3)/(2))) <= ((x^2+y^2)^2)/((x^2+y^2)^((3)/(2))) = sqrt(x^2+y^2)$， \
      故 $f(x,y)->0=f(0,0)$，所以在原点连续。 \
      由定义 \
      $f_x(0,0)=lim_(h->0)(f(h,0)-f(0,0))/h=0$， \
      $f_y(0,0)=lim_(h->0)(f(0,h)-f(0,0))/h=0$，偏导存在。 \
      若 $f$ 在原点可微，则应有余项估计 \
      $(f(x,y)-f(0,0)-f_x(0,0) x-f_y(0,0) y)/(sqrt(x^2+y^2)) -> 0$。 \
      但取 $y=x>0$ 时 \
      $f(x,x)=(x^4)/((2x^2)^((3)/(2)))=(x)/(2sqrt(2))$， \
      $(f(x,x))/(sqrt(2)x)=((x)/(2sqrt(2)))/(sqrt(2)x)=(1)/(4) != 0$， \
      故 $f$ 在 $(0,0)$ 处不可微。
    ],
  ),
)
