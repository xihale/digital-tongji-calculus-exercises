...
#section[总习题一]
#subsection[一、选择题]
1. 当  $x -> 0$  时， $(1 - cos x)^2$  是  $sin^2 x$  的(#answer-choice[]) (#answer-choice[A]).
   #choices(
   [高阶无穷小],
   [同阶无穷小, 但不是等价无穷小],
   [低阶无穷小],
...
2. 设  $f(x)$  为奇函数, 则下列函数中 (#answer-choice[]) (#answer-choice[D]) 也为奇函数.
   #choices(
   [ $f(x) + C$，其中 $C$ 为非零常数],
   [ $f(-x) + C$，其中 $C$ 为非零常数],
   $f(x) + f(-x)$,
...
3. 设函数  $f(x) = x^2 + arctan(1/(x - 1))$ ，则  $x = 1$  是  $f(x)$  的(#answer-choice[]) (#answer-choice[B]).
   #choices(
   [可去间断点],
   [跳跃间断点],
   [无穷间断点],
   ...
   #answer-process[
     ...
     虽然两个单侧极限都存在且有限，但它们不相等（左极限为 $1 - pi/2$，右极限为 $1 + pi/2$）。
