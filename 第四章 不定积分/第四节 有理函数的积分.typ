...
3. 令  $t = tan(x/2)$ , 则  $integral (tan x)/(sin x + cos x - 1) dif x = integral (A)/((1 - t)(1 - t^2)) dif t$  中  $A = -2$ (#answer-choice[(错误)])
   #answer-process[
     当 $t = tan(x/2)$ 时：
...
4. 在计算三角函数有理式的不定积分  $integral R(sin x, cos x) dif x$  时，一般使用变换  $t = tan(x/2)$ (#answer-choice[(正确)])
   #answer-process[
     这是三角函数有理式的标准处理方法。
   ]
