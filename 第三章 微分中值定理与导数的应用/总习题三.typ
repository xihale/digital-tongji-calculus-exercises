...
#section[总习题三]
#subsection[一、选择题]
1. 设在区间  $[0, 1]$  上  $f''(x) > 0$ ，则下列判断正确的是(#h(2em)).
  #choices(
    $f'(1) > f'(0) > f(1) - f(0)$,
    $f'(1) > f(1) - f(0) > f'(0)$,
    $f(1) - f(0) > f'(1) > f'(0)$,
  ...
  #answer-space
2. 设  $f'(x_0) = f''(x_0) = 0, f'''(x_0) > 0$ ，则(#h(2em)).
  #choices(
    [$f'(x_0)$  是  $f'(x)$  的极大值],
    [$f(x_0)$  是  $f(x)$  的极大值],
    [$f(x_0)$  是  $f(x)$  的极小值],
  ...
  #answer-space
#subsection[二、填空题]
3. 函数  $y = ln sin x$  在区间  $[pi/6, (5pi)/6]$  上满足罗尔中值定理的  $xi$  值是 #blank()

4. $lim_(x->0)(e^x + e^(-x) - 2)/x^2 = #blank()$

5.曲线  $y = x e^(-x)$  的拐点是 ，凸区间是 #blank()

6. 函数  $f(x) = 8 ln x - x^2$  在区间  $(0, +infinity)$  上的最大值是 #blank()

7. 曲线  $f(x) = e^x/(x + 1)$  的渐近线为 #blank()

8.抛物线  $y = x^2 - 4x + 3$  在其顶点处的曲率为 #blank()
