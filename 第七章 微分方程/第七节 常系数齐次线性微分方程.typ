#import "../template.typ": *

#section[第七节 常系数齐次线性微分方程]
#subsection[一、选择题]
1. 设线性无关的函数  $y_1, y_2, y_3$  都是二阶非齐次线性微分方程  $y'' + P(x) y' + Q(x) y = f(x)$  的解， $C_1, C_2, C_3$  是任意常数，则该微分方程的通解是(#h(2em)).
   #choices(
   [$C_1 y_1 + C_2 y_2 + C_3 y_3$],
   [$C_1 y_1 + C_2 y_2 - (C_1 + C_2) y_3$],
   [$(1 + C_1 + C_2) y_1 + C_1 y_2 + C_2 y_3$],
   [$(1 + C_1 + C_2) y_1 - C_1 y_2 - C_2 y_3$]
   )
#subsection[二、填空题]
2. 设  $y_1 = cos x$  与  $y_2 = sin x$  是微分方程  $y'' + y = 0$  的两个解，则该微分方程的通解为 #blank()

3. 微分方程  $y'' - 2y' + y = 0$  的通解为 #blank()

4. 已知  $y = e^x$  与  $y = e^2x$  是某二阶常系数齐次线性微分方程的两个解，则该微分方程为 #blank()

#subsection[三、计算题]
5. 求下列微分方程的通解：

   (1)  $y'' + y' - 2y = 0$
   #answer-space
   (2)  $y'' - 4y' + 5y = 0$.
   #answer-space
6. 求下列微分方程满足所给初值条件的特解：

   (1)  $y'' - 3y' - 4y = 0, y|_(x = 0) = 0, y'|_(x = 0) = -5$;
   #answer-space
   (2)  $y'' - 4y' + 13y = 0, y|_(x=0) = 0, y'|_(x=0) = 3$.
   #answer-space
#subsection[四、应用题]
7. 设圆柱形浮筒的底面直径为  $0.5  m$ , 将它铅直地放在水中, 当稍向下压后突然放开, 浮筒在水中上下振动的周期为  $2  s$ , 求浮筒的质量.
   #answer-space
#subsection[五、证明题]
8. 验证:  $y = C_1 x^2 + C_2 x^2 ln x$  ( $C_1, C_2$  是任意常数) 是微分方程  $x^2 y'' - 3x y' + 4y = 0$  的通解.
#answer-space
9. 验证:  $y = 1/x (C_1 e^x + C_2 e^(-x)) + e^x/2 (C_1, C_2$  是任意常数)是微分方程  $x y'' + 2y' - x y = e^x$  的通解.
#answer-space