...
#section[总习题五]
#subsection[一、选择题]
1. 设  $I = integral_(a)^(b) f(x) , dif x$ ，根据定积分的几何意义可知( ).
   #v(0.5em)
   #choices(
...
4. 设函数  $f(x)$  连续， $x > 0$ ，且  $integral_(1)^(x^2) f(t) , dif t = x^2 (x - 1)$ ，则  $f(2) = (quad)$ .
   #v(0.5em)
   #choices(
   $(3 sqrt(2))/2 - 1$,
   $2 sqrt(2) - {12}$,
...
#subsection[二、填空题]
6.  $d/d x integral_(a)^(b) arctan x dif x = underline(quad)$  
7.  $integral_(-pi/2)^(pi/2) sqrt(1 - cos^2 x) dif x =$

8. 由区间  $[a, b]$  上连续曲线  $y = f(x)$ , 直线  $x = a, x = b (a < b)$  和  $x$  轴所围成图形的面积为  $S = #blank()$ .

9.  $integral_(-1)^0 |3x + 1| dif x =$  
10. 已知  $x e^x$  为函数  $f(x)$  的一个原函数, 则  $integral_(0)^(1) x f'(x) , dif x =$
