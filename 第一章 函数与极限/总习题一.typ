...
#section[总习题一]
#subsection[一、选择题]
1. 当  $x -> 0$  时， $(1 - cos x)^2$  是  $sin^2 x$  的(#answer-choice[A]).
   #choices(
   [高阶无穷小],
   [同阶无穷小, 但不是等价无穷小],
   [低阶无穷小],
...
2. 设  $f(x)$  为奇函数, 则下列函数中 (#answer-choice[D]) 也为奇函数.
   #choices(
   [ $f(x) + C$，其中 $C$ 为非零常数],
   [ $f(-x) + C$，其中 $C$ 为非零常数],
   $f(x) + f(-x)$,
...
3. 设函数  $f(x) = x^2 + arctan(1/(x - 1))$ ，则  $x = 1$  是  $f(x)$  的(#answer-choice[B]).
   #choices(
   [可去间断点],
   [跳跃间断点],
   [无穷间断点],
   ...
   #answer-process[
     ...
     当 $x -> 1^+$ 时，$1/(x - 1) -> +oo$，所以 $arctan(1/(x - 1)) -> pi/2$，
...
#subsection[三、计算题]
8. 求下列极限：
   (1)  $lim_(x-> +oo)x(sqrt(x^2 + 1) -x)$
   #answer-process[
     使用分子有理化的方法：
     $lim_(x-> +oo)x(sqrt(x^2 + 1) - x) = lim_(x-> +oo) x dot (sqrt(x^2 + 1) - x)(sqrt(x^2 + 1) + x)/(sqrt(x^2 + 1) + x)$
     
     $= lim_(x-> +oo) x dot (x^2 + 1 - x^2)/(sqrt(x^2 + 1) + x)$
     
     $= lim_(x-> +oo) x/(sqrt(x^2 + 1) + x)$
     
     分子分母同时除以 $x$（注意 $x > 0$）：
     
     $= lim_(x-> +oo) 1/(sqrt(1 + 1/x^2) + 1)$
     
     $= 1/(sqrt(1 + 0) + 1) = 1/2$
   ]
   
   
   (2)  $lim_(x->oo)((2x + 3)/(2x + 1))^(x + 1)$
   #answer-process[
     将表达式改写为：
     $lim_(x->oo)((2x + 3)/(2x + 1))^(x + 1) = lim_(x->oo)((1 + 2/(2x + 1))^(x + 1))$
     
     $= lim_(x->oo)(((1 + 2/(2x + 1))^((2x + 1)/2))^(2(x + 1)/(2x + 1)))$
     
     由于 $lim_(x->oo)(1 + 2/(2x + 1))^((2x + 1)/2) = e$，
     
     而 $lim_(x->oo) 2(x + 1)/(2x + 1) = lim_(x->oo) (2x + 2)/(2x + 1) = 1$，
     
     因此 $lim_(x->oo)((2x + 3)/(2x + 1))^(x + 1) = e^1 = e$。
   ]
...
10. 证明:  $lim_(n->oo)(1/sqrt(n^2 + 1) + 1/sqrt(n^2 + 2) + dots + 1/sqrt(n^2 + n)) = 1$.
     #answer-process[
       ...
       对左端求极限：
       $lim_(n->oo) n/sqrt(n^2 + n) = lim_(n->oo) n/(n sqrt(1 + 1/n)) = lim_(n->oo) 1/sqrt(1 + 1/n) = 1$
       
       对右端求极限：
       $lim_(n->oo) n/sqrt(n^2 + 1) = lim_(n->oo) n/(n sqrt(1 + 1/n^2)) = lim_(n->oo) 1/sqrt(1 + 1/n^2) = 1$
       
       根据夹逼准则，$lim_(n->oo) S_n = 1$。
     ]
