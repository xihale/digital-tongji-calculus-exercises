...
#let problems = (
  (
    ...
    kind-title: [一、选择题],
    stem: [
      函数 $f(x) = cases(cos((pi x)/(l)) & 0 <= x <=(l)/(2), 0 & l/(2)< x <= l)$ 展开成余弦级数时，应对 $f(x)$ 进行
    ],
    options: (
      [周期为 $2 l$ 的延拓],
      [偶延拓],
  ...
  (
    kind: "choice",
    stem: [
      函数 $f(x) = cases(2x + 1 & -3 <= x <= 0, x & 0 < x <= 3)$ 展开成傅里叶级数时，应
    ],
    options: (
    ...
    answer: [A],
    solution: [
      函数定义在 $[-3,3]$ 上，只需作周期延拓（周期为 6）。因 $f(0^+) = 0 != f(0) = 1$，$x = 0$ 是间断点，级数在 $x = 0$ 处收敛于左右极限的均值而不等于 $f(x)$，故仅在 $(-3, 0) union (0, 3)$ 内收敛于 $f(x)$，在端点 $x = plus.minus 3$ 处收敛于均值。选项 A 正确。
    ],
  ),
  (
    kind: "blank",
    kind-title: [二、填空题],
    stem: [
      将函数 $f(x) = cases(0 & 0 <= x <= 1, 2x & 1 < x <= 2)$ 在 $[0, 2]$ 上展开成余弦级数，其和函数在 $x=1$ 处的值为
    ],
    answer: [$1$],
    solution: [
