...
#let problems = (
  (
    ...
    answers: ([√], [√], [√]),
    solution: [
      (1) √。$D$ 的顶点为 $(1,1)$、$(3,1)$、$(3,3)$，既可表示为 $X$ 型区域 $1 <= x <= 3, 1 <= y <= x$，也可表示为 $Y$ 型区域 $1 <= y <= 3, y <= x <= 3$。 \
      (2) √。按 $X$ 型区域，积分限确为 $1 <= x <= 3, 1 <= y <= x$。 \
      (3) √。$ I = int_1^3 dif x int_1^x 1/(y ln x) dif y = int_1^3 1/(ln x) [ln y]_1^x dif x = int_1^3 dif x = 2 $
    ],
  ),
  (
    kind: "choice",
