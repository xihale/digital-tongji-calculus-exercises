...
#let problems = (
  ...
  (
    ...
    answer: [B],
    solution: [
      绝对值级数 $tilde 1/n$ 发散；将相邻两项配对
      $1/(2k) - 1/(2k-1) = -1/(2k(2k-1))$，配对级数收敛，且通项趋于 $0$，
      故原级数收敛，为条件收敛。
    ],
  ),
  (
  ...
  (
    ...
    answer: [D],
    solution: [
      $D=D_1 union D_2$：$y in [0,2]$ 时 $x: sqrt(y) -> sqrt(8-y^2)$；
      $y in [2,4]$ 时 $x: sqrt(y) -> 2$。故为 D。
    ],
  ),
  ...
  (
    ...
    solution: [
      比式审敛得 $R=1$，端点 $x=0,2$ 发散，收敛域 $(0, 2)$。
      由 $sum_(n=0)^oo t^n = 1/(1-t)$（$|t|<1$）两边对 $t$ 微分两次：
      $sum_(n=0)^oo (n+1)(n+2) t^n = 2/(1-t)^3$。
      令 $t=x-1$，得 $S(x) = 2/(2-x)^3$。
    ],
  ),
  (
    kind: "compute",
    ...
    solution: [
      由高斯公式，$div F = 3(x^2+y^2+z^2)$（$f$ 相关项抵消）。
      立体为锥 $x=sqrt(y^2+z^2)$ 与单位球所围，球坐标
      $rho in [0,1]$，$phi in [0, pi/4]$，$theta in [0, 2pi]$，
      $iiint 3 rho^2 · rho^2 sin phi dif rho dif phi dif theta
      = 3 · (1/5) · 2pi · integral_0^(pi/4) sin phi dif phi
      = (3/5)(2-sqrt(2)) pi$。
    ],
  ),
  (
  ...
  (
    ...
    solution: [
      当 $g(0,0)=0$ 时：$f'_x(0,0)=f'_y(0,0)=0$，
      $|Delta f|/rho <= sqrt(2) |g| -> 0$，故可微。
      当 $g(0,0)!=0$ 时：沿 $y=0$ 有
      $lim_(h -> 0) (|h| g(h,0))/h$ 左右极限反号，故 $f'_x(0,0)$ 不存在
      （$f'_y$ 同理），从而不可微。
    ],
  ),
)
