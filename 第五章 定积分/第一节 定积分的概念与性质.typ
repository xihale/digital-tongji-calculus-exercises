...
#subsection[一、判断题(如果错误，请加以改正)]
1.  $(d integral_a^b f(x) dif x)/(d x) = f(x)$  (#answer-choice[])  

2. 定积分的定义中，“ $lambda -> 0$ ”可以换成“ $n -> infinity$ ”. (#answer-choice[])

3.交换定积分的上下限，定积分的值不变. (#answer-choice[])

4.若等式  $integral_a^b f(x) dif x = integral_a^c f(x) dif x + integral_c^b f(x) dif x$  成立，则必有  $a <   c <   b.$  (#answer-choice[])
