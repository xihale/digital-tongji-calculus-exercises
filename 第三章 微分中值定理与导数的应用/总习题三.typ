...
5.曲线  $y = x e^(-x)$  的拐点是 $(2, 2e^(-2))$，凸区间是 $(-oo, 2)$ , 凹区间是 #blank[ $(2, +oo)$]
   #answer-process[
     $y' = e^(-x) - x e^(-x) = (1-x) e^(-x)$
...
6. 函数  $f(x) = 8 ln x - x^2$  在区间  $(0, +oo)$  上的最大值是 #blank[$8 ln 2 - 4$]
   #answer-process[
     $f'(x) = 8/x - 2x$
...
7. 曲线  $f(x) = e^x/(x + 1)$  的渐近线为 #blank[$x = -1$, $y = x - 1$]
  ...
  使用洛必达法则求 $lim_(x->+oo) ((e^x)/(x+1))/x = lim_(x->+oo) (e^x)/(x+1) · 1/x = +oo$
     
  求斜渐近线 $y = k x + b$：
     $k = lim_(x->+oo) f(x)/x = lim_(x->+oo) (e^x)/(x(x+1))$ （趋于无穷，无斜渐近线）
     
     当 $x -> -oo$ 时，$f(x) -> 0$，$y = 0$ 可能是渐近线... 需要重新分析。
...
9. 求下列极限：\
  ...
  (2)  $lim_(x-> +oo)((2/pi) arctan x)^x$.
  #answer-process[
    设 $y = ((2/pi) arctan x)^x$，则 $ln y = x (ln((2)/pi) arctan x)$
    
    当 $x -> +oo$ 时，$arctan x -> pi/2$，所以 $(2/pi) arctan x -> 1$。
    
    设 $u = (2/pi) arctan x$，当 $x -> +oo$ 时 $u -> 1$。
    
    $ln y = x ln u = x ln(1 + (u-1)) ≈ x(u - 1)$ （当 $u -> 1$）
    
    $u - 1 = (2/pi) arctan x - 1 = (2/pi) arctan x - (2/pi) · (pi/2) = (2/pi)(arctan x - pi/2)$
    
    当 $x -> +oo$：$arctan x - pi/2 ≈ -1/x$（由导数知 $arctan'x = 1/(1+x^2) ≈ 1/(x^2)$）
    
    所以 $x(u-1) ≈ x · (2/pi) · (-1/x) = -2/pi$
    
    因此 $lim_(x -> +oo) ((2/pi) arctan x)^x = e^(-2/pi)$
  ]
...
10. 求下列函数在指定点处具有指定阶数及余项的泰勒公式：\
  ...
  #answer-process[
    ...
    泰勒公式：
  $x^3 ln x = (x-1) + 5/2!(x-1)^2 + 11/3!(x-1)^3 + 6/4!(x-1)^4 + (f^(5)(xi)/5!)(x-1)^5$
    
  $= (x-1) + 5/2(x-1)^2 + 11/6(x-1)^3 + 1/4(x-1)^4 - 1/20xi^2(x-1)^5$
  ]

11. 设  $a > 1$ ，函数  $f(x) = a^x - a x$  在区间  $(- oo, + oo)$  上的驻点为  $x(a)$ . 问： $a$  为何值时， $x(a)$  最小？并求出最小值.
   #answer-process[
     $f'(x) = a^x ln a - a = 0$，得 $a^x ln a = a$，$a^x = a / ln a$。
     
     取对数：$x ln a = ln(a / ln a) = ln a - ln(ln a)$
     
  $x = 1 - (ln(ln a)/ln a)$，即驻点 $x(a) = 1 - (ln(ln a)/ln a)$
     
  为使 $x(a)$ 最小，令 $(d x(a)/d a) = 0$：
     
  设 $g(a) = 1 - (ln(ln a)/ln a)$
     
     $d g/d a = -(1/(a ln a) · ln a - ln(ln a) · 1/a/(ln a)^2)$
     
     $= -(1/a - ln(ln a)/a/(ln a)^2) = -(1 - ln(ln a)/a(ln a)^2)$
...
12. 曲线弧  $y = sin x (0 < x < pi)$  上哪一点处的曲率半径最小? 求出该点处的曲率半径.
   #answer-process[
     $y = sin x$，$y' = cos x$，$y'' = -sin x$
      ...
      曲率公式：$K = |y''|/(1 + (y'^2)^(3/2)) = sin x/(1 + cos^2 x)^(3/2)$（在 $0 < x < pi$ 上 $sin x > 0$）
     
     曲率半径：$R = 1/K = (1 + cos^2 x)^(3/2) / sin x$
     
     要使 $R$ 最小，即使 $K$ 最大。
     
     令 $d K/d x = 0$：设分母 $u = (1 + cos^2 x)^(3/2)$，分子 $v = sin x$
     
     $K' = v' u - v u'/u^2 = (cos x(1 + cos^2 x)^(3/2) - sin x · 3/2(1 + cos^2 x)^(1/2) · (-2cos x sin x)/(1+cos^2 x)^3)$
     
     $= (cos x(1 + cos^2 x) + 3 sin^2 x cos x/(1 + cos^2 x)^(5/2))$
     
     $= (cos x(1 + cos^2 x + 3 sin^2 x)/(1 + cos^2 x)^(5/2))$
     
     $= (cos x(1 + cos^2 x + 3(1-cos^2 x))/(1 + cos^2 x)^(5/2))$
     
     $= (cos x(4 - 2cos^2 x)/(1 + cos^2 x)^(5/2))$
...
在区间(0,1)内至少有一个零点.
   #answer-process[
     构造辅助函数：
     $F(x) = a_0 x + a_1 x^2/2 + a_2 x^3/3 + dots + (a_n x^(n+1)/n+1)$
     
     则 $F'(x) = a_0 + a_1 x + a_2 x^2 + dots + a_n x^n = f(x)$
     
     由条件：
     $F(0) = 0$
     $F(1) = a_0 + a_1/2 + a_2/3 + dots + a_n/n+1 = 0$
...
15. 证明：当  $e < a < b < e^2$  时， $ln^2 b - ln^2 a > (4/e^2) (b - a)$ .
   #answer-process[
     ...
     所以 $ln^2 b - ln^2 a = 2 ln xi/xi (b - a)$
     
     需要证明：$2 ln xi/xi > 4/e^2$
     
     设 $g(x) = 2 ln x/x$，求其最小值。
     
     $g'(x) = 2/x · x - 2 ln x/x^2 = 2 - 2 ln x/x^2 = (2(1 - ln x)/x^2)$
     
     令 $g'(x) = 0$：$ln x = 1$，得 $x = e$。
     
     当 $x < e$ 时 $g'(x) > 0$，当 $x > e$ 时 $g'(x) < 0$。
     
  所以 $g(x)$ 在 $x = e$ 处取得最大值，$g(e) = 2/e$。
     
     但我们需要下界。由于 $e < xi < b < e^2$，$g(x)$ 在此区间上的行为：
  - 当 $e < xi < e^2$ 时，$g(xi) = 2 ln xi/xi$
     
     在边界处：
     $g(e) = 2/e ≈ 0.736$
     $g(e^2) = 2 · 2/e^2 = 4/e^2 ≈ 0.541$
     
     由于 $xi in (a,b) subset (e, e^2)$ 且 $g$ 在此区间单调递减，有：
     $2 ln xi/xi > g(e^2) = 4/e^2$
     
     因此 $ln^2 b - ln^2 a > 4/e^2 (b - a)$。
   ]
