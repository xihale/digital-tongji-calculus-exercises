...
2.下列方程中(#answer-choice[C])是一阶微分方程
   ...
   [$x y'' + y' + y = 0$]
    ...
    #answer-process[
       选项 C 的方程 $(x^2 - y^2) d x + (x^2 + y^2) d y = 0$ 仅含一阶微分；
...
6. 写出在点  $(x, y)$  处的切线的斜率等于该点横坐标平方的曲线所满足的微分方程
    #answer-process[
       由题设得切线斜率 $y' = x^2$，故微分方程为 $(d y)/(d x) = x^2$。
    ]
7. 求下列微分方程的通解：

    (1)  $x y' - y ln y = 0$ ;
    #answer-process[
   设 $y > 0$，分离变量得 $(d y)/(y ln y) = (d x)/x$。
   积分可得 $ln|ln y| = ln|x| + C$，吸收绝对值常数，整理为 $ln y = C_1 x$。
   因而通解为 $y = e^(C_1 x)$。
    ]
    (2)  $(e^(x + y) - e^x) d x + (e^(x + y) + e^y) d y = 0$.
    #answer-process[
...
8. 求下列微分方程满足所给初值条件的特解：
    ...
    (1)  $cos x sin y d y = cos y sin x d x, y|_(x = 0) = pi/4$
    #answer-process[
   化为 $(d y)/(d x) = (sin x cos y)/(cos x sin y) = tan x / tan y$，分离变量得 $tan y times d y = tan x times d x$。
...
9. 一曲线通过点  $(2,3)$，且它在两坐标轴间的任一切线均被切点所平分，求该曲线方程
   ...
   方程 $(d y)/(d x) = -y/x$ 分离变量得 $ln y = -ln x + C$，即 $x y = C_1$。
...
10. 一个半球体形状的雪堆, 其体积融化率与半球体面积  $A$  成正比, 比例系数  $k > 0$ . 假设在融化过程中雪堆始终保持半球体形状, 已知半径为  $r_0$  的雪堆在开始融化的  $3  h$  内, 融化了其体积的  $7/8$ , 问: 雪堆全部融化需要多少时间?
      #answer-process[
         对半球体有 $V = (2/3) pi r^3$、$A = 2 pi r^2$，因此 $(d V)/(d t) = 2 pi r^2 times (d r)/(d t)$。
         由题设 $(d V)/(d t) = -k A = -2k pi r^2$，可得 $(d r)/(d t) = -k$，从而 $r(t) = r_0 - k t$。
