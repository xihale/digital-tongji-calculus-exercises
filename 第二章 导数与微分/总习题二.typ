...
#section[总习题二]
#subsection[一、选择题]
1. 设函数  $f(x) = (x - a) phi(x)$ ，其中函数  $phi(x)$  在点  $x = a$  处连续，则必有(#h(2em)).
   #choices(
   [$f'(x) = phi (x)$],
   [$f'(x) = phi (x) + (x - a) phi'(x)$],
   [$f'(a) = phi (a)$],
   ...
   #answer-space
2. 若函数  $y = f(x)$  有  $f'(x_0) = 1/2$ ，则当  $Delta x -> 0$  时该函数在点  $x = x_0$  处的微分  $d y$  是  $Delta x$  的(#h(2em)).
   #choices(
   [同阶无穷小, 但不是等价无穷小],
   [等价无穷小],
   [低阶无穷小],
...
5. 设函数  $y = x/(1 - 2 sin x) - ln (4 - x)$ ，则  $y' |_x = pi =$
   #answer-space
6.曲线  $y = 2x^3 - 5x^2 + 4x - 5$  上点 $(2,-1)$ 处的法线方程是
   #answer-space
